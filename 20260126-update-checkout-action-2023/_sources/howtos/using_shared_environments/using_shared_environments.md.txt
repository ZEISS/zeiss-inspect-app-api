...
Add a key-value pair `"environment": <environment_name>` to the `metainfo.json` files of all Add-ons which you want to share the environment. `<environment_name>` can be any string, but you are advised to use a unique name to avoid potential conflicts with other, separate environments. The `environment` element can be placed at any position within the JSON file.
