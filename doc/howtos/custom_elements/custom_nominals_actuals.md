...
For the complete list of geometric classes and required result structures, refer to the API reference sections:

* <a href="../../python_api/python_api.html#gom-api-extensions-nominals">Extensions API - Nominals</a>
* <a href="../../python_api/python_api.html#gom-api-extensions-actuals">Extensions API - Actuals</a>
...
```{caution}
The element name widget object name `name` is reserved and case-sensitive.
```
...
All custom nominal or actual elements are based on the <a href="../../python_api/python_api.html#gom-api-extensions">Extensions API</a>. More specifically, a custom nominal or actual element class is inherited from the <a href="../../python_api/python_api.html#gom-api-extensions-customelement">CustomElement</a> base class. The older `ScriptedElement` name is available as a legacy alias.

This How-to focuses on implementation flow and practical patterns. For complete constructor signatures and class behavior, refer to the API reference.
...
line 8..9:
: The constructor calls the super class constructor while setting a unique contribution ID and a human-readable description. Refer to the API class reference for full signature details.
...
The `endpoint` identifies the service. Use a unique endpoint string across your Apps and keep it stable after release.
...
line 3:
: Service endpoint identifier (must be unique across installed Apps)
...
See [Using services](../using_services/using_services.md) for details.
...
Create a custom element from a Python script by using function `gom.script.customelements.create_actual()` or `gom.script.customelements.create_nominal()`, respectively.
...
Optional element data can be persisted using the `data` entry of the computation result and retrieved later via element tokens.

#### Storing optional element data
...
#### Retrieving optional element data

The entries stored in `data` are persisted with the element and can be read later via element tokens.
Each key in the `data` dictionary becomes a token on that element.

```{code-block} python
:caption: Reading optional element data from element tokens
:linenos:
...
print(f"source={source}, quality_score={quality_score}")
```

```{caution}
Choose `data` keys carefully: a custom data key must not match an existing attribute/token of the element type.
Use unique, token-friendly names (for example `app_source`, `app_quality_score`, `app_iteration_count`) to avoid collisions.
```
...
Log messages can be created by using the method <a href="../../python_api/python_api.html#gom-api-extensions-customelement-add-log-message">add_log_message()</a>.
...
- No custom element entry appears in the Construct menu
